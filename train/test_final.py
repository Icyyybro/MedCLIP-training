...
def test_image_dataset():
    ...
    try:
        # 测试训练集
        train_dataset = ImageLabelDataset('/root/autodl-tmp/mimic_cxr/datasets.pkl', 'train')
        ...
        print(f"Labels type: {sample['labels'].dtype}")
        print(f"Caption preview: {sample['caption'][:100]}...")
        
        # 测试验证集
        val_dataset = ImageLabelDataset('/root/autodl-tmp/mimic_cxr/datasets.pkl', 'val')
        print(f"\nVal dataset size: {len(val_dataset)}")
        
        val_sample = val_dataset[0]
        print(f"Val labels shape: {val_sample['labels'].shape}")
        print(f"Val labels: {val_sample['labels']}")
...
def test_text_dataset():
    ...
    try:
        dataset = TextLabelDataset('/root/MedCLIP-main/local_data/sentence-label.csv')
